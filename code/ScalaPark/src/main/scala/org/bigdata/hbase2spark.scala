...
object hbase2spark {
  ...
  def batch2feature(sc:SparkContext) {
    val hbaseconf = getHBaseConfiguration("vm","2181")
    ...
    val counterUserId2MovieId = data.filter(x=>x._4==1.0)
      ...
      .flatMapValues(x=>{
        ...
        //        val jedisIns = new JedisIns("vm",6479,100000)
        val jedisIns:Jedis = new Jedis("vm",6479,100000)
        for (record<-x) {
          sum=sum+1
    ...
    counterUserIdPos.foreach( x=> {
      val jedisIns = new Jedis("vm",6479,100000)
